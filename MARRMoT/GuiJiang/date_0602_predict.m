...
%% 1. Prepare data
% Load the data
% predict_input_data = readtable('input_2009_2012.csv');
% predict_input_data = readtable('predict_input_200604_200806.csv');
% predict_input_data = readtable('dataset_input1');
predict_input_data = readtable('eval_cmfd_0606_200604_201112.csv');
% predict_input_data = readtable('pred_0605_200701_200807.csv');
% predict_input_data = readtable('pred_0605_cmfd_era5l_200701_200807.csv');
...
% input_climatology.precip   = predict_input_data.prcp_OBS;                   % Daily data: P rate  [mm/d]
input_climatology.precip   = predict_input_data.prcp_OBS;

% input_climatology.temp     = predict_input_data.t2m;                     % Daily data: mean T  [degree C]
input_climatology.temp     = repelem(nan, length(predict_input_data.date));
input_climatology.pet      = predict_input_data.PET;    % Daily data: Ep rate [mm/d]
...
test = strsplit(model, '_');
Sn = strsplit(test{5}, 's');
Sn = str2double(Sn{1});
input_s0 = repelem(0, Sn);                                                      % Initial slow flow storage [mm]
...
[output_ex,...                                                             % Fluxes leaving the model: simulated flow (Q) and evaporation (Ea)
 ...
 output_waterbalance] = ...                                                % Water balance check              
                        m.get_output();
writematrix(output_ex.Q, ['D:/YJS/QIHANG/waterSci/waterSci/data/processed/matlab_res/eval_cmfd_0606_', model, '.csv'])
%% 6. Analyze the outputs                   
% Prepare a time vector
t = datenum(predict_input_data.date, 'yyyy-mm-ddTHH:MM:SSZ');
